	...
	JSON, utils)
{
    ...
    function getFixedFileDate(
        ...
        if (Files.exists(inFilePath)) {
            ...
            var file     = utils.getFile(files.convertURLToOSPath(inFilePath, true)),
                fileDate = file[inDate],
                fileTime = fileDate.getTime(),
                date     = utils.getDate(fileTime);
    ...
    return files = {
        ...
        read: function (inFilePath) {
            ...
            var filePath = this.convertURLToOSPath(inFilePath, true);
            ...
            if (len === -1) {
                ...
                try {
                    ...
                    if (myFile && typeof myFile.close === 'function')
                        myFile.close();
                }


                alert("File.read: Invalid File: " + filePath + "\n\n" + inFilePath +  "\n\nTest: " + myFile);
                //console.log(JSON.stringify(file, null, "    "));
            }
            //utils.log("Files: Reading " + len + "b File: " + filePath + "\nIn Path: " + inFilePath);
         ...
         */
        readChunks: function (inFilePath,
                              inChunkSize,
                              inCallback) {
            inFilePath = this.path(inFilePath);
        ...
        write: function (inFilePath,
            ...
            var file = Files.open(inFilePath, true, inIncludeBOM
                ? "UTF8"
                : "");
        ...
        append: function (inFilePath,
            ...
            var file   = utils.getFile(this.convertURLToOSPath(inFilePath, true)),
                result = false;
        ...
        /**
         ...
         file doesn't exist.
         */
        getCreatedDate: function (inFilePath) {
        ...
        /**
         ...
         file doesn't exist.
         */
        getModifiedDate: function (inFilePath) {
         ...
         */
        getSize: function (inFilePath,
                           inUnit) {
            inFilePath = this.convertURLToOSPath(this.path(inFilePath), true);

            var size      = utils.getFile(inFilePath).length,
                power     = {
                    B:  0,
                    KB: 1,
                    MB: 2,
         ...
         */
        copyDirectoryContents: function (inFromPath,
                                         inToPath) {
         ...
         */
        getCurrentScriptURL: function (inFunction) {
            var url = "";
        ...
        /**
         ...
         defined `inFunction`.
         */
        getCurrentScriptDirectory: function (inFunction) {
         ...
         */
        getCurrentScriptFilename: function (inFunction) {
         ...
         */
        convertURLToOSPath: function (inURL,
                                      inDontQuote) {
        ...
        createDirectories: function(path) {
            var parts = path.replace(/file:\/\/\/([A-Z])\|/, "$1:").split("/");
            var directory = "file:///" + parts[0];
            //utils.log("Creating Directory: \n *** " + path + "\n *** " + parts.join("/"));
            var created = 0;
            for (var i = 1, j = parts.length; i < j; i++) {
                const name = parts[i];
                //directory = Files.makePathFromDirAndFile(directory, name);
                directory += '/' + name;
                if (Files.isDirectory(directory))
                    continue;
                Files.createDirectory(directory);
                created++;
                //utils.log("Created Directory: " + directory);
            }
            return created;
        }
    };
