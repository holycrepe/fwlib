...
// =======================================================================
         ...
         */
        FwPanel.prototype.getToggleItem = function (title, options) {
         ...
         */
        FwPanel.prototype.getTextItem = function (title, options) {
            var defaultName = getDefaultName(title);
            ...
            return {
                TextInput: {
                    name:    options.name,
                    width:   this.getTextInputSize(options.numLabels),
                    toolTip: options.description,
                    events:  {
        ...
        FwPanel.prototype.getToggleFormItem = function (title, options) {
            ...
            return {
                FormItem: {
                    label:    title + ":",
                    children: [
                        this.getToggleItem(options.target || title, options),
                    ],
                },
            };
        };
        ...
        FwPanel.prototype.getFormItem = function (title, options) {
            ...
            for (var i = 0; i < totalToggles; i++) {
                ...
                var itemOptions = toggles[i];
                //noinspection JSDuplicatedDeclaration
                var item        = this.getToggleItem(itemOptions.title || title, itemOptions);
                children.push(item);
            }

            {
                ...
                //noinspection JSDuplicatedDeclaration
                var item = this.getTextItem(itemOptions.title || title, itemOptions);
                children.push(item);
            }
        ...
        FwPanel.prototype.getTextInputSize = function (numLabels) {
            if (typeof numLabels === 'undefined')
                numLabels = 1;
            return this.options.inputWidth - ((numLabels - 1) * this.options.labelWidth);
        };
