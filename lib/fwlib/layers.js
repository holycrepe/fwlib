...
define([
    ...
    utils
        ...
        var layers = {
            ...
            // =======================================================================
            /**
             Creates new LayerTree object and returns all elements

             @param {Fw.FwDocument} [dom=currentDOM] The document to get elements from.  If this isn't included, the current document is
             used as the source.
             @param {LayerElementDataOptions} [options]
             @returns {Fw.FwElement[]}
             @memberof module:layers
             */
            getElements: function(dom, options) {
                var data = layers.getElementData(dom, options);
    ...
    // =======================================================================
    /**
     Gets LayerElementData from Elements

     @param {Fw.FwElement[]} elements Elements.
     @returns {LayerElementData}
     */
    function getElementDataFromElements(elements) {

        var names = _.map(elements, function (element) {
            return element.symbolName || element.name
        });
        return {
    ...
    */
    ...
    LayerTree.prototype = /** @lends module:layers.LayerTree.prototype */ {
        ...
        */
        ...
        // ===================================================================
        /**
         An array of Elements representing all of the elements in the
         current state of the current page.
        ...
        */
        ...
        // ===================================================================
        /**
         Get Elements from all layers

         @param {LayerElementDataOptions} [options]
         @returns {Fw.FwElement[]} Elements in all layers.
         */
        getElements: function(options) {
            var layers = this.layers;
            if (options && options.reverseLayers) {
                layers.reverse();
            }
            return _
                .chain(layers)
        ...
        // ===================================================================
        /**
         Get Element Data from all layers

         @param {LayerElementDataOptions} [options]
         @returns {LayerElementData} Element Data for all layers.
         */
        getElementData: function(options) {
            return getElementDataFromElements(this.getElements(options));
        },
    };
    ...
    */
        ...
        inConfig)
    {
        if (typeof inConfig == "object") {
            ...
            for (var attribute in inConfig) {
                if (hasOwnProperty.call(inConfig, attribute))
    ...
    /**
     ...
     @memberof module:layers
     */
    layers.getElements = function(dom, options) {
        var data = layers.getElementData(dom, options);
        return data.elements;
    };
    ...
    // =======================================================================
    /**
     Creates new LayerTree object and returns elements data

     @param {Fw.FwDocument|Fw.FwElement[]} [elementsOrDom=currentDOM] An Element array, or the document to get elements from.  If this isn't included, the current document is used as the source.
     @param {LayerElementDataOptions} [options] Options: Include Web Layers. Ignored if element array is explicitly passed as first argument.
     @returns {LayerElementData}
     @memberof module:layers
     */
    layers.getElementData = function(elementsOrDom, options) {
        options = options || {};
        //alert("layers: Get Element Data: \n" + utils.stringify(options));
        var ignoreWebLayers = options.includeWebLayers !== true;
        //alert("layers: Get Element Data: \n" + ignoreWebLayers);
        if (typeof elementsOrDom !== 'undefined' && utils.isArray(elementsOrDom)) {
            return getElementDataFromElements(elementsOrDom);
        }
        var tree = new layers.LayerTree(elementsOrDom, ignoreWebLayers);
        return tree.getElementData(options);
    };
    ...
    /**
        ...
        @param {Fw.FwDocument} inTargetDom The document to copy the layer's elements to.
        @param {Fw.FwDocument} [inSourceDom=currentDOM] The document to copy the layer's
