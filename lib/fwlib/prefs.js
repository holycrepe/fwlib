    ...
    JSON) 
{
    ...
    /**
        ...
        @param {PrefsStorageOptions} options
    ...
    */
		...
		options)
    {
        ...
        try {
            ...
            var preferencesJson = fw.getPref(inName);
			prefs = eval("(" + preferencesJson + ")") || {};
			if (options.debug) {
                alert("Loaded preferences for " + inName + " from JSON.\n\n" + preferencesJson);
            }
        } catch (exception) {
            if (options.debug) {
                alert("Loading default preferences for " + inName + ".");
            }
            prefs = inDefaults;
            return prefs;
        }
    ...
    */
    function set(
        ...
        inPrefs)
    {
        //console.log("Saved preferences for " + inName);
    ...
    /**
        ...
        @param {Object} [inDefaults=null] An optional object containing default
            properties that will be added to the stored preference data, if any.
        @param {PrefsStorageOptions} [options] An optional object containing options
            for DomStorage.
        ...
        */
            ...
            inSource)
        {
            for (var name in inSource) {
                if (!inSource.hasOwnProperty(name))
                    continue;
		...
		mixin(this, get(_metadata.name, inDefaults, options));
    }
