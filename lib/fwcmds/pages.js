	...
	utils)
{
    ...
    function add(options) {
        var logger = utils.getLogger('Pages: Add Page', 3),
            log = logger.log;
        
        if (!fw.documents.length) {
            logger.warn("Skipping; No Documents Open");
            return;
        }
        ...
        if (options.name) {
            log("Setting Page Name: " + options.name);
            dom.pageName = options.name;
        }

        // update the new page with properties from the original page and
        // the settings in the dialog
        log("Setting Export State");
        ...
        if (options.templatePath) {
            ...
            if (options.insertClipboard) {
                ...
                log("Pasting Clipboard Contents");
                dom.clipPaste();
        ...
        switch (options.location) {
            ...
            case PageLocation.Before:	// before
                log("Moving Before Current Page");
