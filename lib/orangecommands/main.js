...
// a collection of (hopefully) useful tools for Fireworks
            ...
            var defineElementAccessors = function (module, propertyNames) {
                _.defaults(module, {
                    ...
                    getSymbolInfo:  function (options, includeWebLayers) {
                        var elements = module.getElements(includeWebLayers || (options && options.includeWebLayers));
                        return Elements.getSymbolInfo(elements, options);
                    },
                    getSymbols:     function (options, includeWebLayers) {
                        var elements = module.getElements(includeWebLayers || (options && options.includeWebLayers));
                    ...
                    if (naming.folder) {
                        if (!naming.isAbsoluteFolder) {
                            ...
                            if (outputDirRegex) {
                                ...
                                folders.path = folders.path.replace(outputDirRegex, '');
                            }
                            var folderTemplate = naming.folder;
                            naming.folder = path + naming.folder;
            ...
            /**
             * @param callback
             * @param [options] {OrangeCommands.Pages.PageDataOptions}
             **/
            function pagesWithPage(callback, options) {
                var page = Page.getInfo(options);
                callback.call(this, page);
            }
            /**
             * @param callback
             * @param [options] {OrangeCommands.Pages.PageEnumerationOptions}
             **/
            function pagesEach(callback, options) {
                options = options || {};
                if (options.currentOnly)
                    return pagesWithPage(callback, options.data);
                ...
                for (var i = start; i < max; i++) {
                    ...
                    var page = Page.getInfo(options.data);
            ...
            var Pages = Pages || {
                ...
                withPage:     function (callback, options) {
                    pagesWithPage(callback, options);
                },
            ...
            defineElementAccessors(Pages, ['count', 'info', 'pageElements', 'pageElementsList', 'summary']);
            ...
            var PageState = function PageState(options) {
                ...
                var
                    /** @type {PageData} **/
                    data = this,
                    elementData = Document.getElementData();
                _.assign(data, {
                    name: info.name,
            ...
            Page.getElementData = function (includeWebLayers) {
                return Document.getElementData(includeWebLayers);
            };
            ...
            var Elements  = Elements || {
                ...
                getSymbol:function (element, options) {
                    ...
                    var hasSameName = info.elementName === info.name;
                    ...
                    if (options.synchronizeNames) {
                        if (hasSameName) {
                            ...
                            utils.log("Renaming " + info.elementName + " to " + info.name);
                            fw.selection = element;
                            var dom      = fw.getDocumentDOM();
                            dom.setElementName(info.name);
                        }
                    }
                    return info;
                },
