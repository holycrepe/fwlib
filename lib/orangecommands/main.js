...
// a collection of (hopefully) useful tools for Fireworks
             ...
             */
            var defineGetters = function (obj, propertyNames) {
                for (var i = 0, j = propertyNames.length; i < j; i++) {
                    var propertyName = propertyNames[i];
                    var funcName = 'get' + utils.capitalizeFirstLetter(propertyName);
            ...
            var defineElementAccessors = function (module, propertyNames) {
                _.defaults(module, {
                    getElements:    function (includeWebLayers) {
                        var data = module.getElementData(includeWebLayers);
                        return data.elements;
                    },
                    getSymbolInfo:  function (includeWebLayers) {
                        var elements = module.getElements(includeWebLayers);
                        return Elements.getSymbolInfo(elements);
                    },
                    getSymbols:     function (includeWebLayers) {
                        var elements = module.getElements(includeWebLayers);
                        return Elements.getSymbols(elements);
                    }
                });

                propertyNames = (propertyNames || []).concat(['elementData', 'elements', 'symbolInfo', 'symbols']);
                defineGetters(module, propertyNames);
            ...
            var Document = Document || {
                ...
                getElementData: function (includeWebLayers) {
                    return fwLayers.getElementData(undefined, includeWebLayers);
                },
                ...
                export_pages:             function (options) {
                    ...
                    options             = Document.processExportPagesOptions(options);
            ...
            defineElementAccessors(Document);

            function pagesWithPage(callback, asPageElements) {
                var page = Page.getInfo(asPageElements);
                callback.call(this, page);
            }

            function pagesEach(callback, options, asPageElements) {
                /** @type {OrangeCommands.Pages.PageEnumerationOptions} **/
                options = options || {};
                if (options.currentOnly)
                    return pagesWithPage(callback, asPageElements);
            ...
            var Pages = Pages || {
                ...
                each:         function (callback, options) {
                    pagesEach(callback, options)
                },
                eachPageElements:         function (callback, options) {
                    pagesEach(callback, options, true)
                },
                withPage:     function (callback) {
                    pagesWithPage(callback);
                },
                withPageElements:     function (callback) {
                    pagesWithPage(callback, true);
                },
            ...
            defineElementAccessors(Pages, ['count', 'info', 'pageElements', 'pageElementsList']);
            ...
            var PageState = function PageState() {
                ...
                _.assign(data, {
                    ...
                    index: info.index,
                    isMaster: Document.is_master_page(),
                    elements: Selection.getAll()
                });
                _.assign(data, {
                    elementsData: fwLayers.getElementData(data.elements),
                    symbols: Elements.getSymbols(data.elements)
                });
                _.assign(data, {
                    symbolInfo: data.symbols,
                    elementCount: data.elementData.count
                });
            };
            ...
            Page.getInfo = function (asPageElements) {
                return asPageElements
                    ? new PageState()
                    : new Page();
            };
            ...
            Page.setExportFormat = function (options) {
                ...
                if (typeof options === 'string') {
                    ...
                    if (_formatPresets.hasOwnProperty(formatName)) {
                        exportOptions = _formatPresets[formatName];
                        setMatte      = true;
                    }
                } else {
            ...
            /** @returns {LayerElementData} **/
            Page.getElementData = function () {
                return fwLayers.getElementData(Selection.current);
            };

            defineElementAccessors(Page, ['elementCount']);
            ...
            var Elements  = Elements || {
                getSymbolInfo: function (elements) {
                    return _.filter(_.map(elements, function (element, full) {
                        if (typeof full === 'undefined')
                            full = false;
                        //noinspection JSValidateTypes
                        /** @type {OrangeCommands.Elements.SymbolInfo} **/
                        var info = {
                            id: element.symbolID,
                        };
                        if (!info.id)
                            return false;
                        info.name        = element.symbolName;
                        info.elementName = element.name;
                        if (!full) {
                            if (info.elementName === info.name) {
                                delete info.elementName;
                            }
                        }
                        return info;
                    }));
            ...
            var Selection = Selection || {
                ...
                join:             function (delimiter) {
                    ...
                    Selection.each(function (/** Element **/ field) {
                        if (field.is_text()) {
            ...
            defineElementAccessors(Selection, ['all', 'current']);
...
// Utility methods
            ...
            Element.__prototype__.is_group  = function () {
                //noinspection EqualityComparisonWithCoercionJS
                return (this == "[object Group]");
            };
            Element.__prototype__.is_symbol = function () {
                return (this.kind() === 'symbol');
            };
            Element.__prototype__.is_text   = function () {
                return (this.kind() === 'text');
            };
            Element.__prototype__.kind      = function () {
                var element = this;
