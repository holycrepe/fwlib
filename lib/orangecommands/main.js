...
// a collection of (hopefully) useful tools for Fireworks
                ...
                defineElementAccessors: function (module, propertyNames) {
                    _.defaults(module, {
                        ...
                        getSymbolInfo:  function (options) {
                            var elements = module.getElements(options && options.layers);
            ...
            var Document = Document || {
                ...
                importPages:        function (path, options) {
                    path = path || "file:///J|/\u0024\u0024/Symbols/IO/Containers/Folders/Graphic.png";
                    /** @type {ImportPagesOptions} **/
                    options = options || {};
                    var range = PagesInternals.getRange(options.range);
                    var start = options.start || 0;
                    var end = options.hasOwnProperty('end')
                        ? options.end
                        :  start + (options.max || 10000);
                    for (var i=range.start; i<range.end;i++) {
                        var pageNumber = i+1;
                        utils.log("Importing Page #" + i);
                        try {
                            Document.importPage(path, i);

                        } catch (ex) {
                            utils.log("Error Importing Page #" + i + ": " + ex);
                            break;
                        }
                    }
                },
                 ...
                 *
                 * @param path {string}
                 * @param [pageNumberOrOptions] {number|ImportPageOptions}
                 */
                importPage:        function (path, pageNumberOrOptions) {
                    /** @type {ImportPageOptions} **/
                    var options = typeof pageNumberOrOptions === 'number'
                        ? {
                    ...
                    dom.InsertPageForImport(path, pageNumber);
                    dom = fw.getDocumentDOM();
                    dom.importFile(path, options.boundingRectangle, options.maintainAspectRatio, pageNumber, options.insertAfterCurrentPage);
            ...
            OcInternals.defineElementAccessors(Document);
            ...
            var Elements  = Elements || {
                ...
                getSymbol:function (element, options) {
                    ...
                    //noinspection JSValidateTypes
                    var hasSameName = info.elementName === info.name || info.elementName === null;
            ...
            OcInternals.defineElementAccessors(Selection, ['all', 'current']);
