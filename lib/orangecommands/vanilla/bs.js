...
get_export_folder_name = function () {
...
Document = {
    ...
    export_pages_in:          function (options) {
        ...
        options.folder = options.folder || orangecommands.params;
        options.promptFolder = true;
    ...
    export_pages_with_prefix:          function (options) {
        ...
        options.prefix = options.prefix || orangecommands.params;
        options.promptPrefix = true;
    ...
    export_pages:          function (options) {
        ...
        if (!options.folder) {
            if (options.promptFolder) {
                options.folder = fw.browseForFolderURL("Choose an export folder",null);
                options.isAbsoluteFolder = true;
            } else {
                options.folder = get_export_folder_name();
            }
        }
        if (!options.prefix) {
            if (options.promptPrefix) {
                options.prefix = prompt("Prefix:",null);
            }
        }
        var folder = options.folder;
        if (!options.isAbsoluteFolder) {
            folder = Document.path() + folder;
        }

        Files.createDirectory(folder);

        Pages.each(function () {
            if (Document.is_master_page()) {
            ...
            Document.export_page(options.format);
        });
    },
    ...
    export_page:          function (options) {
        if (!Document.is_saved()) {
            alert("You need to save the document first!");
            return;
        }
        //console.log("Exporting Page X1a...");
        //console.log("Exporting Page X1b...");
        var export_file_path =  Document.get_export_page_name();
        //console.log("Exporting Page X2...");
        //alert("Exporting Page: " + export_file_path);
        Document.export_in(export_file_path, options);
    },
    ...
    export_in:            function (path, options) {
        ...
        Document.set_export(options);
        fw.exportDocumentAs(null, path, null);
    },
    set_export:            function (options) {
    ...
    get_export_page_name: function(options) {
        ...
        //log("get_export_page_name...");
        options = options || {};
        if (!options.page)
            options.page = fw.getDocumentDOM().pageName;
    ...
    get_export_file_name: function(options) {
        //log("get_export_file_name...");
        options = options || {};
        var export_file_path = options.file;
        if (!export_file_path) {
            //log("get_export_file_name === ");
            export_file_path = fw.getDocumentPath(null).split('fw.png')[0].split('.png')[0];
        }
        if (options.page)
            export_file_path += "_" + options.page + ".png";
        return export_file_path;
    },
    is_master_page:       function() {
        var dom = fw.getDocumentDOM();
        return dom.hasMasterPage() && dom.currentPageNum == 0
    },
    is_saved:             function () {
